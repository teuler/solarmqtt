...
'   tftp 192.168.178.110 get "SolarMQTT_v13.bas"
' ----------------------------------------------------------------------------
Option Explicit
Option Base 0
Option Escape
...
Sub readWeekLog drv$
  ...
  Open FNAME_WEEK$ For Random As #2
  nRec = Int(Lof(#2) /DAY_RECLEN)
  If DEBUG>1 Then Print "nRec=";nRec
  If nRec > 0 Then
    ' Read first entry
    Seek #2, 1
    dat$ = Input$(DAY_RECLEN-1, #2)
    If DEBUG>1 Then Print "First entry='";dat$;"'"
    log.first_energy = Val(Field$(dat$, 2, ","))
    If DEBUG>1 Then Print "->log.first_energy=";log.first_energy
    log.nWeek = 0
    For i=0 To N_DAYS -1
      iRec = nRec -(N_DAYS -i)
      If iRec > 0 Then
        If DEBUG>1 Then Print "i=";i;" iRec=";iRec
        Seek #2, iRec *DAY_RECLEN +1 +iRec
        dat$ = Input$(DAY_RECLEN-1, #2)
        If DEBUG>1 Then Print "dat$='";dat$;"'"
    ...
    Next i
  EndIf
  Print "-> " +Str$(log.nWeek +1) +" record(s) read."
  Close #2
End Sub
    ...
    '  ' Read values ...
      For i=1 To nRec-1
       ...
       'Seek #1, i *DAY_RECLEN +i ' 2023-09-28 ??
        ...
        Else
          Print "ERROR: entry ";i;" seems damaged"
          Print Len(s$), Asc(dat$)
        EndIf
        Inc log.nDay, 1
      Next i
...
Sub UpdateDisplay
  ...
  If DEBUG>1 Then Print "log.nWeek=";log.nWeek
  For i=0 To N_DAYS -1
    x1 = x +i*(dx+4)
    Box x1,y,dx,dy3, 1,cbk,cbk
    If DEBUG>1 Then Print "i=";i
    s$ = log.wdays$(i)
    If Len(s$) > 0 Then
      If i = N_DAYS -1 Then
        ' Today ...
        enrgy = energy_day
        If Debug>1 Then Print "'";s$;"' (today), energy_day=";energy_day
      Else
        ' Earlier ...
        enrgy = log.energy(i+1) -log.energy(i)
        If Debug>1 Then Print "'";s$;"' enrgy=";enrgy
      EndIf
